...
def setup_net(net, ip_tun=True, quic_tun=True, gdb=False, tcpdump=False, multipath=False, web_cmd='python3 -m http.server 80 &'):
    ...
    if quic_tun:
        if gdb:
            ...
            net['cl'].cmd('./picoquicdemo -P plugins/datagram/datagram.plugin  10.2.2.2 4443 2>&1 > log_client.log &'.format(plugins))
    sleep(1)
    net['web'].cmd(web_cmd)
    sleep(1)
...
def run():
    net_cleanup()
    net = Mininet(KiteTopo(bw_a=100, bw_b=100, delay_ms_a=10, delay_ms_b=10, loss_a=3, loss_b=3), link=TCLink, autoStaticArp=True, switch=OVSBridge, controller=None)
    net.start()

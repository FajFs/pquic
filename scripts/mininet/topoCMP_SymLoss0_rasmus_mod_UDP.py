...
def replaceSubStringInFile(subStringToReplace, newSubString, filename):
    
    #Read in file
    with open(filename) as f:
        content = f.readlines()

    #Go through each line and replace string
    modifiedLines = []
    for line in content:
        newLine = line.replace(subStringToReplace, newSubString)
...
def run():
    ...
    for i in range(var_range[0], var_range[1], var_step_length):
        ...
        replaceSubStringInFile(str(currentValue) + "m", str(i*2) + "m", "../scripts/mininet/auto_iperf_udp.sh") 
        currentValue = i*2
    ...
    name = "udp_" + var_name + str(var_range[0]) + "_" + str(var_range[1]) + "_" + str(random.randint(5,10000))
    print("Filename: " + name)

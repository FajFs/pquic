...
class KiteTopo(Topo):
    def build(self, **opts):
        #delay=None, jitter=None,loss=None, max_queue_size=None
        wifi_opt = {'delay': '500ms', 'loss': 0, 'max_queue_size': 37, 'bw': 1, 'jitter': '0ms'}
        lte_opt = {'delay': '500ms', 'loss': 0, 'max_queue_size': 37, 'bw': 1, 'jitter': '0ms'}
...
def run():
    net_cleanup()
    net = Mininet(KiteTopo(bw_a=1000, bw_b=1000, delay_ms_a=10, delay_ms_b=10, loss_a=0, loss_b=0), link=TCLink, autoStaticArp=True, switch=OVSBridge, controller=None)
    net.start()
